...
# Manages web application and scheduler as separate services
...
case "$1" in
    ...
    setup)
        ...
        if [ ! -d "$VENV_DIR" ]; then
           ...
           #pip3.8 install python-dotenv psycopg2-binary gunicorn flask paramiko apscheduler flask-sqlalchemy sqlalchemy requests python-dateutil
    ...
    *)
        ...
        echo "  status           - Show status of services, (tail -f)"
