...
from scheduler_config import ProductionScheduler
...
def cleanup_temporary_log_files():
    ...
    try:
        ...
        # Define patterns for temporary files to clean up
        temp_patterns = [
            'aliases_test_*.txt',
            'simple_collection_test*.json',
        ...
        for pattern in temp_patterns:
            ...
            for temp_file in temp_files:
                try:
                    # Keep app.log but clean other temporary files
                    if not temp_file.endswith('app.log'):
                        os.remove(temp_file)
...
def scheduled_backup_job():
    ...
    try:
        with app.app_context():
            ...
            backup_filename = f'switch_analyzer_backup_{timestamp}.sql.gz'
            backup_path = os.path.join(backup_dir, backup_filename)

            # Get database URL
            db_url = os.getenv('DATABASE_URL')
            if not db_url:
                logger.error("Scheduled backup failed: DATABASE_URL not configured")
                return
            ...
            if result.returncode == 0:
                ...
                logger.info(f"Scheduled backup completed successfully: {backup_filename} ({stat.st_size} bytes)")
...
def scheduled_collection_job(username=None, password=None):
    """Execute a scheduled collection using clean system"""
    try:
        with app.app_context():
    ...
    try:
        with app.app_context():
            ...
            try:
                ...
                logger.warning(f"Could not clear scheduler jobs: {e}")
...
def delete_backup(filename):
    ...
    try:
        ...
        if not (filename.endswith('.sql') or filename.endswith('.sql.gz')):
            return jsonify({'error': 'Only SQL backup files can be deleted'}), 400
